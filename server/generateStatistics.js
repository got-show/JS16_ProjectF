...
PlodC.generate = async function() {
  ...
  function plodAge() {
    ...
    for (var i in characters) {
      ...
      var age = !char.alive && char.hasOwnProperty('death')
        ? char.death - birth
        : BOOK_YEAR - birth;
      var plod = char.plodB;
    ...
    for (var j in distribution) {
      ...
      returnArray.push([
        distribution[j].l + "-" + distribution[j].u,
        avg * 100
      ]);
    }
  ...
  function plodDistribution() {
    ...
    console.log(temp);
    console.log("plodDistribution finished. All " + temp.length + " data points were generated using " + (
